...
const planMap =
  process.env.NODE_ENV === "development"
    ? new Map([
        ["price_1L3tbaGVXzqW0iHkEAZ9XCrO", "free"],
        ["price_1L3tbaGVXzqW0iHkNO8mU8Pb", "starter"],
        ["price_1L3tbaGVXzqW0iHkr69zv8hH", "unlimited"],
      ])
    : // Production prices
      new Map([
...
paymentRouter.post(
  "/create-checkout-session/:priceId/:email",
  async (req: Request, res: Response) => {
    ...
    if (session.url == null)
      return res
        .status(401)
        .send({ error: "Unable to create checkout session" });
...
paymentRouter.post(
  "/create-customer-portal-session/:stripeCustomerId",
  async (req, res) => {
