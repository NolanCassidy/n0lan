...
functionRouter.post("/v1/explain", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { prompt, stop, postFormat } = isLongerThanOneLine
      ? EXPLAIN_WITH_LANGUAGE(codeTrimmed, inputLanguage)
      : EXPLAIN_SIMPLE(codeTrimmed, inputLanguage);
    ...
    if (responseIsLooped) {
      const {
        prompt: backupPrompt,
        stop: backupStop,
        postFormat: backupPostFormat,
      } = EXPLAIN_SIMPLE(codeTrimmed, inputLanguage);

      const backupCodexResponse = await axios.post(
        CODEX_ENDPOINT,
        {
          prompt: backupPrompt,
          temperature: CODEX_TEMPERATURE,
  ...
  try {
    throwErrorOnEmpty(
      ...
      },
      {
        ...
        errorOnEmpty: "Output language has not been selected",
      }
    );
    throwErrorOnCondition({
      condition: code.length > MAX_CODE_LENGTH,
    ...
    const { prompt, stop, postFormat } = TRANSLATE(
      code,
      inputLanguage,
      outputLanguage
    );
...
functionRouter.post("/v1/docstring", async (req: Request, res: Response) => {
  return res.status(200).send({
    output: "This version of the docstring function has been depreciated",
  });
});
...
functionRouter.post("/v2/docstring", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const parametersRes = await axios.post(
    ...
    const returnPromise = axios.post(
      CODEX_ENDPOINT,
      {
...
Args:
${
  extractedParamsExplained != null
    ? extractedParamsExplained
        .map((param) => `  ${param.param}: ${param.explained}`)
        .join("\n")
    : "  None"
}
Returns:
...
functionRouter.put("/feedback/:id", async (req, res) => {
  ...
  try {
    ...
    const updatedResult = await Fig.findOneAndUpdate(
