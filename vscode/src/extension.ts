...
export function activate(context: vscode.ExtensionContext) {
  ...
  const login = vscode.commands.registerCommand("fig.login", async () => {
    vscode.env.openExternal(
      vscode.Uri.parse(getLoginURL(vscode.env.uriScheme))
    );
  });

  const logout = vscode.commands.registerCommand("fig.logout", async () => {
    vscode.env.openExternal(
      vscode.Uri.parse(getLogoutURL(vscode.env.uriScheme))
    );
  });

  const askFunction = vscode.commands.registerCommand("fig.ask", async () => {
    const question = await vscode.window.showInputBox({
      title: "What would you like to ask about the selected code?",
          ...
          if (editor?.selection) {
            ...
            try {
              ...
              const askResponse = await axios.post(
                `${BACKEND_ENDPOINT}/function/v1/ask`,
                {
                  code: highlight,
  ...
  const explainFunction = vscode.commands.registerCommand(
    "fig.explain",
    async () => {
      vscode.window.withProgress(
        {
            ...
            if (editor?.selection) {
              ...
              try {
                ...
                const explainResponse = await axios.post(
                  `${BACKEND_ENDPOINT}/function/v1/explain`,
                  {
                    code: highlight,
                    inputLanguage: editor.document.languageId,
                ...
                potentiallyReplaceTokens(newTokens);
                // Add language here
                const commentedExplain = addComments(
                  output,
  ...
  const askFileFunction = vscode.commands.registerCommand(
    "fig.askFile",
    async () => {
      const question = await vscode.window.showInputBox({
        title: "What would you like to ask about the file?",
        placeHolder: "Enter you question here",
      });
  ...
  const docstringFunction = vscode.commands.registerCommand(
    "fig.docstring",
    async () => {
      vscode.window.withProgress(
        {
        ...
        () => {
          return new Promise(async (resolve, reject) => {
            ...
            if (editor?.selection) {
              ...
              try {
                const { refreshToken, accessToken } = getTokens();
                const docstringResponse = await axios.post(
                  `${BACKEND_ENDPOINT}/function/v2/docstring`,
                  {
                    code: highlight,
                    inputLanguage: editor.document.languageId,
                    accessToken,
                ...
                potentiallyReplaceTokens(newTokens);
                const docstringExplain = addComments(
                  output,
                  editor.document.fileName
                );
                const snippet = new vscode.SnippetString(
  ...
  const complexityFunction = vscode.commands.registerCommand(
    "fig.complexity",
    async () => {
      vscode.window.withProgress(
        {
            ...
            if (editor?.selection) {
              ...
              try {
                ...
                const complexityResponse = await axios.post(
                  `${BACKEND_ENDPOINT}/function/v1/complexity`,
                  {
                    code: highlight,
                    inputLanguage: editor.document.languageId,
                    language: languageId,
